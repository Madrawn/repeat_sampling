from lib_repeat_sampling import hijacker, global_state, neutral_prompt_parser
...
def combine_denoised_hijack(
    ...
    for batch_i, (prompt, cond_indices) in enumerate(zip(global_state.prompt_exprs, batch_cond_indices)):
        ...
        denoised[batch_i] = cfg_cond * get_repeat_denoise_strength_factor(cfg_cond, uncond[batch_i] + cond_delta + aux_cond_delta)
...
def get_repeat_denoise_strength_factor(cfg_cond, cond):
    global_state.apply_and_clear_repeat_denoise_strength_override()
    return global_state.repeat_denoise_strength * (torch.std(cond) / torch.std(cfg_cond) - 1) + 1
